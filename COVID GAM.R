...
dat3<- dat %>%
  filter(year = "2021-%m-%d"))


date <- as.POSIXct(format = "%Y-%m-%d")
...
dat2<- subset(dat1, format(as.Date(date),"%Y")==2022)
plot(dat2)
library(lubridate)
...
plot(cases ~ date, data = dat2, pch = 19, col = "grey50", type = "p", ylab = "Number of Cases", xlab = "Month") # plots the observed values
